            ...
            <div class="list-group-item">
                <div class="d-flex">
                    ...
                    <div class="p-2">
                        ...
                        if ($btn_cad) {
                            echo "<a href='" . pg . "/cadastrar/sts_cad_pagina' class='btn btn-dark btn-sm'>Cadastrar</a>";
                        }
                        ?>
                    </div>
                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <span class="d-none d-md-block">
                                                ...
                                                if ($btn_vis) {
                                                    echo "<a href='" . pg . "/visualizar/sts_vis_pagina?id=" . $row_pg['id'] . "' class='btn btn-dark btn-sm'>Visualizar</a> ";
                                                }
                                                $btn_edit = carregar_btn('editar/sts_edit_pagina', $conn);
                                                if ($btn_edit) {
                                                    echo "<a href='" . pg . "/editar/sts_edit_pagina?id=" . $row_pg['id'] . "' class='btn btn-dark btn-sm'>Editar </a> ";
                                                }
                                                $btn_apagar = carregar_btn('processa/sts_apagar_pagina', $conn);
                                                if ($btn_apagar) {
                                                    echo "<a href='" . pg . "/processa/sts_apagar_pagina?id=" . $row_pg['id'] . "' class='btn btn-dark btn-sm' data-confirm='Tem certeza de que deseja excluir o item selecionado?'>Apagar</a> ";
                                                }
                                                $btn_ordem_menu = carregar_btn('processa/proc_sts_ordem_menu', $conn);
                                                if ($btn_ordem_menu) {
                                                    if ($qnt_linhas_exe == 1) {
                                                        echo "<button type='button' class='btn btn-sm btn-dark disabled'>";
                                                        echo "<i class='fas fa-angle-double-up'></i>";
                                                        echo "</button> ";
                                                    } else {
                                                        echo "<a href='" . pg . "/processa/proc_sts_ordem_menu?id=" . $row_pg['id'] . "'><button type='button' class='btn btn-sm btn-dark'>";
                                                        echo "<i class='fas fa-angle-double-up'></i>";
                                                        echo "</button></a> ";
                                                    }
                                                    $qnt_linhas_exe++;
